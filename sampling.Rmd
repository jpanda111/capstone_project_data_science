...
if (!file.exists(paste(full_path, "Good_Turing_5.trim",trim,".rds", sep=""))) {
  ...
  remove(freqfreq1,freqfreq2,freqfreq3); gc()
...
# KBO cannot trim freq > 4 since needs to calculate prob and leftovers
if (!file.exists(paste(full_path, "gram.trim",trim,".prob.rds", sep=""))) {
  ...
  getProb_trigram <- function(full_path) {
    g <- readRDS(paste(full_path, "gram3", ".trim", trim, ".rds", sep = ""))
    g$discount = rep(1, nrow(g))
    g[freq<=5, discount:= GTCount[freq+1,4]]
    g[, prob:= freq*discount/sum(freq), by=w]
    g[, leftover_prob:= 1-sum(prob), by=w]
    setkey(g,"w")
    setorderv(g, c("w", "prob"), c(1,-1))
    saveRDS(g, paste(full_path, "gram3", ".trim", trim, ".prob.rds", sep = ""), ascii=FALSE, compress=TRUE)
    g5 <- g[, .(pred,prob,leftover_prob), by=w]
    saveRDS(g5, paste(full_path, "gram3", ".trim", trim, ".probtop5.rds", sep = ""), ascii=FALSE, compress=TRUE)
    rm(g,g5); gc()
  }
  
  getProb_bigram <- function(full_path) {
    g <- readRDS(paste(full_path, "gram2", ".trim", trim, ".rds", sep = ""))
    g$discount = rep(1, nrow(g))
    g[freq<=5, discount:= GTCount[freq+1,3]]
    g[, prob:= freq*discount/sum(freq), by=w]
    g[, leftover_prob:= 1-sum(prob), by=w]
    setkey(g,"w")
    setorderv(g, c("w", "prob"), c(1,-1))
    saveRDS(g, paste(full_path, "gram2", ".trim", trim, ".prob.rds", sep = ""), ascii=FALSE, compress=TRUE)
    rm(g,g5); gc()
  }
  
  getProb_unigram <- function(full_path) {
    g <- readRDS(paste(full_path, "gram1", ".trim", trim, ".rds", sep = ""))
    g$discount = rep(1, nrow(g))
    g[freq<=5, discount:= GTCount[freq+1,2]]
    g[, prob:= freq*discount/sum(freq), by=word]
    setkey(g,"word")
    setorderv(g, c("word", "prob"), c(1,-1))
    saveRDS(g, paste(full_path, "gram1", ".trim", trim, ".prob.rds", sep = ""), ascii=FALSE, compress=TRUE)
    rm(g,g5); gc()
  }
  
  getProb_trigram(full_path)
  getProb_bigram(full_path)
  getProb_unigram(full_path)
...
if (!file.exists(paste(full_path,  "shiny-app/train_stupid_back_off.db", sep = ""))) {
...
for (n in 1:4) {
  ...
  sql <- paste("INSERT INTO NGRAM VALUES ($w, $pred, $score, ", n, ")", sep = "")
  dbBegin(db)
...
if (!file.exists(paste(full_path,  "train_katz_back_off_trigram.db", sep = ""))) {
mydrv <- dbDriver("SQLite");
db <- dbConnect(drv=mydrv, dbname=paste(full_path, "train_katz_back_off_trigram.db", sep = ""))
...
g5 <- readRDS(paste(full_path, "gram3.trim1.probtop5.rds",sep=""))
sql <- paste("INSERT INTO NGRAM VALUES ($w, $pred, $prob, $leftover_prob)", sep = "")
dbBegin(db)
dbSendPreparedQuery(db, sql, bind.data = data.frame(g5))
dbCommit(db)
dbDisconnect(db)

db <- dbConnect(drv=mydrv, dbname=paste(full_path, "train_katz_back_off_bigram.db", sep = ""))
...
g5 <- readRDS(paste(full_path, "gram2.trim1.prob.rds",sep=""))
sql <- paste("INSERT INTO NGRAM VALUES ($w, $pred, $prob, $leftover_prob, $freq, $discount)", sep = "")
dbBegin(db)
dbSendPreparedQuery(db, sql, bind.data = data.frame(g5))
dbCommit(db)
dbDisconnect(db)

db <- dbConnect(drv=mydrv, dbname=paste(full_path, "train_katz_back_off_unigram.db", sep = ""))
...
g5 <- readRDS(paste(full_path, "gram1.trim1.prob.rds",sep=""))
sql <- paste("INSERT INTO NGRAM VALUES ($word, $prob, $freq, $discount)", sep = "")
dbBegin(db)
dbSendPreparedQuery(db, sql, bind.data = data.frame(g5))
dbCommit(db)
dbDisconnect(db)
